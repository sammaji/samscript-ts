...
class Interpreter {
  ...
  private evaluateForStmt(
    body: Stmt,
    initializer?: Expr | VarDecl,
    condition?: Expr,
    update?: Expr,
  ) {
    if (initializer) {
    ...
    switch (expr.operator.type) {
      ...
      case TokenType.PLUS:
        ...
        throw new RuntimeError(
          "Operands must be both numbers or one must be string",
          expr.operator,
        );
      case TokenType.MINUS:
        this.isNumberOperands(expr.operator, left, right);
  ...
  private evaluateLogicalExpr(expr: BinaryExpr) {
    ...
    if (
      (expr.operator.type === TokenType.OR && left) ||
      (expr.operator.type === TokenType.AND && !left)
    ) {
      return left;
    }
